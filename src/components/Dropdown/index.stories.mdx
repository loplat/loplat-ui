...
export const customTriggerSource = `
<Dropdown.Trigger>
  <IconButton>
    <MoreVertIcon />
  </IconButton>
...
<Source dark language="tsx" code={customTriggerSource} />
...
export const optionWithLabelSource = `
<Dropdown.OptionList>
  {options.map((value, index) => {
           const loading = index % 2 === 1;
           return (
...
<Source dark language="tsx" code={optionWithLabelSource} />
...
<Source dark language="tsx" code={disabledSource} />
      ...
      <Dropdown.OptionList>
        {options.map((value) => {
          ...
          return (
            <Dropdown.Option
              value={value}
              label={<Checkbox name="checkbox" label={value} checked={selected} />}
              selected={selected}
              key={value}
            />
          );
...
<Source dark language="tsx" code={multipleSource} />
```
