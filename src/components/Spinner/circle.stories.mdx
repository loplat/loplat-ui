...
<Canvas>
  <Story name="small" args={{ duration: 1000, scale: 0.5 }}>
    {Template.bind({})}
  </Story>
</Canvas>
...
<Canvas>
  <Story name="slow" args={{ duration: 1600, scale: 0.5 }}>
    {Template.bind({})}
  </Story>
</Canvas>
...
`<CircleSpinner />` 컴포넌트의 `role` 은 `progressbar` 입니다.
...
`<CircleSpinner />` 컴포넌트가 *특정 영역*의 진행상태를 설명하는 경우, `aria-describedby`를 사용하여 *특정 영역*의 상태임을 표기해야합니다.
