...
import { DiamondIcon, CheckCircleOutlineIcon, WarningCircleOutlineIcon } from '../../../assets/Icon/';
...
export const Template = (args) => {
  ...
  return (
    <div>
      ...
      <Popup1 {...args} isOpen={isOpen} onClose={close} onClickButton={() => {}} />
    </div>
  );
};
...
`title`혹은 `children` 둘 중 하나는 입력하셔야 합니다.
...
<Canvas>
  <Story
    ...
    args={{
      title: '말 파란 별 까닭입니다.',
      children: '이름자를 가슴속에 위에도 당신은 아직 덮어 있습니다. 밤이 위에도 아이들의 까닭입니다.',
      variant: 'primary',
    }}
  >
    {Template.bind({})}
...
아래 예제는 title, children, variant, icon을 여러 방식으로 바꾼 것입니다.

### only title, variant: danger
...
<Story
  ...
  args={{
    ...
    variant: 'danger',
    children: '별 북간도에 소녀들의 파란 언덕 까닭이요, 어머니, 거외다. 사랑과 풀이 벌레는 별 까닭입니다.',
  }}
>
  {DialogTemplate.bind({})}
</Story>

### only content, variant: primary

children만 작성하면 검정색으로 정해집니다.

<Story name="only content" args={{ variant: 'primary', children: '새겨지는 별이 이름을 된 이웃 거외다.' }}>
  {DialogTemplate.bind({})}
</Story>

### too long content, variant: primary
