...
<Meta
  ...
  component={Input}
  argTypes={{
    value: { description: '`input`상태를 연결해주세요.' },
    error: { description: '`true`일 때, `errorMessage`가 하단에 표시됩니다.', table: { category: 'error' } },
    errorMessage: { table: { category: 'error' } },
    type: { description: '브라우저에서 지원하는 `input` 태그의 type 기능입니다.' },
    autoComplete: { description: '브라우저에서 지원하는 `input` 태그의 autoComplete 기능입니다.' },
    inputProps: { control: { type: null } },
    ref: { control: { type: null } },
    id: { description: '`label`태그와 연동하기 위한 `id`프로퍼티입니다.' },
    ...pickPropsInObj(icon, 'isIconVisible', 'rightIcon'),
    ...pickPropsInObj(eventHandler, 'onEnter', 'onBlur', 'onFocus', 'onChange'),
  }}
/>
