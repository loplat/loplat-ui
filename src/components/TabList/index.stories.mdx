...
<Meta
  ...
  argTypes={{
    ...
    tabs: {
      ...generateTable({
        type: "{ value: 'string'; label?: 'string | React.ReactElement'; isDisabled?: 'boolean'; }[]",
      }),
    },
    onChange: {
      ...generateTable({
        type: '(event: React.SyntheticEvent, value: string) => void',
      }),
    },
  }}
/>
...
`tabs` prop의 `label`에 `string` 값 또는 `React.ReactElement` 컴포넌트를 넣을 수 있습니다.
...
export const TabListWithA11y = ({ tabs, ...args }) => {
  const [selectedValue, setSelectedValue] = useState(tabs[0].value);
  return (
    <div>
      <TabList
      ...
      <div
        ...
        style={{ display: selectedValue === tabs[0].value ? 'block' : 'none', padding: 16 }}
        tabIndex={0}
      >
        tab panel 1
      </div>
      <div
        ...
        style={{ display: selectedValue === tabs[1].value ? 'block' : 'none', padding: 16 }}
        tabIndex={0}
      >
        tab panel 2
      </div>
